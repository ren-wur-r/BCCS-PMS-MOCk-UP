...
import {
  type MbsBscHttpGetManyProjectStoneReqMdl,
  type MbsBscHttpGetManyProjectStoneRspItemMdl,
} from "@/services/pms-http/basic/basicHttpFormat";
import { getManyBasicProjectStone } from "@/services";
...
const employeeProjectStoneID = defineModel<number | null>("employeeProjectStoneID");
const employeeProjectStoneName = defineModel<string | null>("employeeProjectStoneName");

// -------------------------------------------------------------------
/** ComboBox 頁面模型 */
interface GetManyProjectStoneComboBoxViewMdl {
  pageIndex: number;
  keywordQuery: string;
  searchingKeyword: string;
  projectStoneList: GetManyProjectStoneComboBoxItemMdl[];
  hasMoreData: boolean;
...
/** ComboBox 項目模型 */
interface GetManyProjectStoneComboBoxItemMdl {
  employeeProjectStoneID: number;
  employeeProjectStoneName: string | null;
}

/** 頁面物件 */
const viewObj = reactive<GetManyProjectStoneComboBoxViewMdl>({
  pageIndex: 1,
  keywordQuery: "",
  searchingKeyword: "",
  projectStoneList: [],
...
const getData = async (pageIndex: number, keyword: string) => {
  ...
  const requestData: MbsBscHttpGetManyProjectStoneReqMdl = {
    employeeLoginToken: tokenStore.token,
    employeeProjectID: props.employeeProjectID,
    employeeProjectStoneName: keyword,
    pageIndex,
  };

  const responseData = await getManyBasicProjectStone(requestData);

  if (!responseData || responseData.errorCode !== MbsErrorCodeEnum.Success)
    return { filteredList: [], hasMore: false };

  const originalCount = responseData.employeeProjectStoneList?.length || 0;

  const filteredList =
    responseData.employeeProjectStoneList?.map(
      (item: MbsBscHttpGetManyProjectStoneRspItemMdl) =>
        ({
          employeeProjectStoneID: item.employeeProjectStoneID,
          employeeProjectStoneName: item.employeeProjectStoneName,
        }) satisfies GetManyProjectStoneComboBoxItemMdl
    ) ?? [];
...
const onInput = debounce(async () => {
  ...
  employeeProjectStoneID.value = null;
  employeeProjectStoneName.value = null;

  viewObj.pageIndex = 1;
  viewObj.projectStoneList = [];
  viewObj.hasMoreData = true;

  const { filteredList, hasMore } = await getData(viewObj.pageIndex, inputValue);

  viewObj.projectStoneList.push(...filteredList);
...
const scrollDropdown = async (event: Event) => {
  ...
  if (nearBottom && viewObj.hasMoreData && !viewObj.isLoading) {
    ...
    const uniqueNewList = filteredList.filter(
      (newItem) =>
        !viewObj.projectStoneList.some(
          (existing) =>
            existing.employeeProjectStoneID === newItem.employeeProjectStoneID
        )
    );

    viewObj.projectStoneList.push(...uniqueNewList);
    viewObj.hasMoreData = hasMore;
...
// -------------------------------------------------------------------
/** 選取項目 */
const selectItem = (item: GetManyProjectStoneComboBoxItemMdl) => {
  viewObj.keywordQuery = item.employeeProjectStoneName || "";

  employeeProjectStoneID.value = item.employeeProjectStoneID;
  employeeProjectStoneName.value = item.employeeProjectStoneName;
...
/** 同步外部 v-model 名稱 */
watch(
  () => employeeProjectStoneName.value,
  (newValue) => {
    viewObj.keywordQuery = newValue || "";
    if (!newValue) employeeProjectStoneID.value = null;
  },
  { immediate: true }
);
</script>
...
<template>
  <div v-click-outside="closeDropdown" class="relative flex w-full">
    ...
    >
      ...
      >
        <ul v-if="viewObj.projectStoneList.length === 0">
          <li class="px-3 py-2 text-sm text-gray-500">查無資料</li>
        </ul>

        <ul v-else>
          <li
            v-for="item in viewObj.projectStoneList"
            :key="item.employeeProjectStoneID"
