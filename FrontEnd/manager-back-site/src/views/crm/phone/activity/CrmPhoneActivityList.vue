...
import { getManyPhoneActivity } from "@/services/index";
...
/** 取得列表 */
const getList = async () => {
  const allRecords = (mockDataSets.crmActivities ?? []).map((item) => ({
    managerActivityID: item.id,
    managerActivityName: item.name,
    managerActivityKind: item.kind as DbAtomManagerActivityKindEnum,
    ...
    managerActivityRegisteredCount: item.registeredCount,
    managerActivityTransferredToSalesCount: item.transferredToSalesCount ?? 0,
  })) as CrmActivityActivityListItemMdl[];

  const rangeStartDate = activityTimeRange.value ? activityTimeRange.value[0] : null;
  const rangeEndDate = activityTimeRange.value ? activityTimeRange.value[1] : null;
  const activityName = crmActivityActivityListViewObj.query.managerActivityName?.trim() ?? "";
  const activityKind = crmActivityActivityListViewObj.query.managerActivityKind;

  let filtered = allRecords;
  if (rangeStartDate) {
    filtered = filtered.filter(
      (item) =>
        new Date(item.managerActivityStartTime) >=
        new Date(formatToServerDateStartISO8(rangeStartDate))
    );
  }
  if (rangeEndDate) {
    filtered = filtered.filter(
      (item) =>
        new Date(item.managerActivityStartTime) <=
        new Date(formatToServerDateEndISO8(rangeEndDate))
    );
  }
  if (activityName) {
    filtered = filtered.filter((item) => item.managerActivityName.includes(activityName));
  }
  if (activityKind) {
    filtered = filtered.filter((item) => item.managerActivityKind === activityKind);
  }

  const startIndex =
    (crmActivityActivityListViewObj.query.pageIndex - 1) *
    crmActivityActivityListViewObj.query.pageSize;
  const endIndex = startIndex + crmActivityActivityListViewObj.query.pageSize;
  crmActivityActivityListViewObj.managerActivityList = filtered.slice(startIndex, endIndex);
  crmActivityActivityListViewObj.totalCount = filtered.length;
  return;
  ...
  // 呼叫 API
  const requestData: MbsCrmPhnHttpGetManyActivityReqMdl = {
  ...
  const responseData: MbsCrmPhnHttpGetManyActivityRspMdl | null =
    await getManyPhoneActivity(requestData);
  if (!responseData) {
  ...
  crmActivityActivityListViewObj.managerActivityList =
    responseData.managerActivityList?.map(
      (item: MbsCrmPhnHttpGetManyActivityRspItemMdl) =>
        ({
          ...
          managerActivityExpectedLeadCount: item.managerActivityRegisteredCount,
...
const handleRowClick = (id: number) => {
  ...
  router.push(`/crm/phone/activity/detail/${id}/pipeline`);
};
//#endregion
...
<template>
  <div class="flex flex-col h-[calc(100vh-100px)] overflow-hidden gap-4 p-2">
    ...
    <div class="flex mb-0 gap-y-4">
      <button class="tab-btn active">活動轉電銷</button>
      <button class="tab-btn" @click="router.push('/crm/phone/pool')">
        電銷名單
      </button>
    </div>
        ...
        <table class="table-base table-fixed table-sticky w-full">
          <thead class="sticky top-0 bg-white z-10">
            <tr>
              ...
              <th class="text-start w-48">活動名稱</th>
          ...
          <tbody>
            ...
            <template v-else>
              ...
              >
                <td class="text-start">{{ formatDateTime(item.managerActivityStartTime) }}</td>
                <td class="text-start">{{ formatDateTime(item.managerActivityEndTime) }}</td>
                <td class="text-start">{{ item.managerActivityName }}</td>
                <td class="text-center">
                  {{ getManagerActivityKindLabel(item.managerActivityKind) }}
                </td>
                <td class="text-end">{{ item.managerActivityExpectedLeadCount }}</td>
                <td class="text-end">{{ item.managerActivityRegisteredCount }}</td>
                <td class="text-end">{{ item.managerActivityTransferredToSalesCount }}</td>
              </tr>
            </template>
          </tbody>
        </table>
